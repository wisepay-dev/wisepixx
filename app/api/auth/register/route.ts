...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ user, message: "Conta criada com sucesso. Bem-vindo à WisePix." }, { status: 201 });
  } catch (error) {
    console.error("register_error", error);
    return NextResponse.json({ error: "Não foi possível criar a conta agora. Tente novamente em instantes." }, { status: 500 });
  }
}
